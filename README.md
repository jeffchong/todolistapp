...
![Release](https://img.shields.io/badge/Release-0.2%20alpha-orange)
...
This is an alpha macOS app. Version `0.2` is intended for early testers who are comfortable with a local-first productivity app that may still change shape.
...
- [0.2 alpha release notes](docs/RELEASE_NOTES_0.2_ALPHA.md)
...
For the `0.2` alpha, use a signed Release build from Xcode when sharing with testers.

1. Open `ToDoListApp.xcodeproj`.
2. Select the `ToDoListApp` target.
3. Set your development team.
4. Confirm the marketing version is `0.2` and increment the build number if you are replacing a prior tester build.
